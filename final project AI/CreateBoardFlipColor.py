...
class CreateBoardFlipColor:
    ...
    def __init__(self):
        ...
        CreateBoardFlipColor.random_choose_cords()

        self.screen_update(True)

    @staticmethod
    def random_choose_cords():
        illegal_choose_cords1 = [(3, 3), (3, 4), (4, 3), (4, 4)]
        for i in range(4):
            x, y = random.randint(0, 7), random.randint(0, 7)
            while (x, y) in illegal_choose_cords1:
                x, y = random.randint(0, 7), random.randint(0, 7)
            CreateBoardFlipColor.flipping_cords.append((x, y))
    ...
    def screen_update(self, first_time= False):
        ...
        for x in range(WIDTH):
            for y in range(HEIGHT):
                if self.player == 0:
                    if CreateBoardFlipColor.static_validation(self.board, self.player, x, y):
    ...
    def static_legal_moves(board, player):
        ...
        for x in range(WIDTH):
            for y in range(HEIGHT):
                if CreateBoardFlipColor.static_validation(board, player, x,y):
                    next_board = CreateBoardFlipColor.static_move_helper(board, x, y)
    ...
    def move(self,x,y):
        ...
        # New rule: flip the color if the coordinate is in flipping list
        if (x, y) in CreateBoardFlipColor.flipping_cords:
            if self.player == Player.PLAYER_1:
                self.board[x][y].player = Player.PLAYER_2
            if self.player == Player.PLAYER_2:
                self.board[x][y].player = Player.PLAYER_1

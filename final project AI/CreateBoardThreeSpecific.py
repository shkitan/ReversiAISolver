...
class CreateBoardThreeSpecific:
    player_1_choosen_cords = []
    player_2_choosen_cords = []
    def __init__(self):
        row = [None] * WIDTH
        self.board = []
        # center values
        ...
        CreateBoardThreeSpecific.random_choose_cords()

        # self.screen_update(True)

    @staticmethod
    def random_choose_cords():
        illegal_choose_cords1 = [(3, 3), (3, 4), (4, 3), (4, 4), (0, 0), (7, 7), (0, 7), (7, 0)]
        illegal_choose_cords2 = [(3, 3), (3, 4), (4, 3), (4, 4), (0, 0), (7, 7), (0, 7), (7, 0)]
        for i in range(3):
            x, y = random.randint(0, 7), random.randint(0, 7)
            while (x, y) in illegal_choose_cords1:
                x, y = random.randint(0, 7), random.randint(0, 7)
            CreateBoardThreeSpecific.player_1_choosen_cords.append((x, y))
            illegal_choose_cords1.append((x, y))
        for i in range(3):
            x, y = random.randint(0, 7), random.randint(0, 7)
            while (x, y) in illegal_choose_cords2:
                x, y = random.randint(0, 7), random.randint(0, 7)
            CreateBoardThreeSpecific.player_2_choosen_cords.append((x, y))
            illegal_choose_cords2.append((x, y))
        ...
        for x in range(WIDTH):
            for y in range(HEIGHT):
                if self.player == Player.PLAYER_1:
                    if CreateBoardThreeSpecific.static_validation(self.board, self.player, x, y):
    ...
    def static_legal_moves(board, player):
        ...
        for x in range(WIDTH):
            for y in range(HEIGHT):
                if player == Player.PLAYER_1 and (x,y) in CreateBoardThreeSpecific.player_2_choosen_cords:
                    continue
                if player == Player.PLAYER_2 and (x,y) in CreateBoardThreeSpecific.player_1_choosen_cords:
                    continue
                if CreateBoardThreeSpecific.static_validation(board, player, x,y):
                    next_board = CreateBoardThreeSpecific.static_move_helper(board, x, y)
